...
class AsyncClientTestCase(AbstractClientTestCase, IsolatedAsyncioTestCase):
    client: AsyncKorapayClient
    ...
    async def test_client_can_charge_via_card(self):
        card = Card(
            cvv="123", expiry_year="30", expiry_month="09", number="4084127883172787"
        )
        response = await self.client.charge_via_card(
            reference=str(uuid4()),
    ...
    async def test_client_can_authorize_card_charge(self):
        # TODO: change form using an already processed transaction
        response = await self.client.authorize_card_charge(
    ...
    async def test_client_can_resend_card_otp(self):
        # TODO: change form using an already processed transaction
        response = await self.client.resend_card_otp(
            transaction_reference="KPY-CA-7VbzDPezNP7O9I7",
        )
        self.assertEqual(response.status_code, codes.CONFLICT)

    async def test_client_can_charge_via_bank_transfer(self):
        response = await self.client.charge_via_bank_transfer(
            reference=str(uuid4()),
            customer_email="johndoe@example.com",
            amount=1000,
            currency=Currency.NGN,
    ...
    async def test_client_can_create_virtual_bank_account(self):
        response = await self.client.create_virtual_bank_account(
    ...
    async def test_client_can_get_virtual_bank_account(self):
        response = await self.client.get_virtual_bank_account(
            account_reference="716b4728-7fca-43e8-916a-3a4c0f298165"
        )
        self.assertTrue(codes.is_success(response.status_code))
        self.assertTrue(response.status)

    async def test_client_can_get_virtual_bank_account_transactions(self):
        response = await self.client.get_virtual_bank_account_transactions(
            account_number="1110026499"
        )
        self.assertTrue(codes.is_success(response.status_code))
        self.assertTrue(response.status)

    async def test_client_can_credit_sandbox_virtual_bank_account(self):
        response = await self.client.credit_sandbox_virtual_bank_account(
            account_number="1110026499", amount=1000, currency=Currency.NGN
        )
        self.assertTrue(codes.is_success(response.status_code))
        self.assertTrue(response.status)

    async def test_client_can_charge_via_mobile_money(self):
        response = await self.client.charge_via_mobile_money(
            reference=str(uuid4()),
            customer_email="johndoe@example.com",
            amount=1000,
    ...
    async def test_client_can_authorize_mobile_money_charge(self):
        # TODO: replace use of already processed token
        response = await self.client.authorize_mobile_money_charge(
            reference="KPY-CA-IeO1NKLtkB4k", token="123456"
        )
        self.assertEqual(response.status_code, codes.CONFLICT)

    async def test_client_can_resend_mobile_money_otp(self):
        response = await self.client.resend_mobile_money_otp(
            transaction_reference="KPY-CA-IeO1NKLtkB4k"
        )
        self.assertEqual(response.status_code, codes.CONFLICT)

    async def test_client_can_resend_stk(self):
        response = await self.client.resend_stk(
            transaction_reference="KPY-CA-IeO1NKLtkB4k"
        )
        self.assertEqual(response.status_code, codes.CONFLICT)

    async def test_client_can_authorize_stk(self):
        response = await self.client.authorize_stk(
            reference="KPY-CA-IeO1NKLtkB4k", pin="1234"
        )
        self.assertEqual(response.status_code, codes.CONFLICT)

    async def test_client_can_initiate_charge(self):
        response = await self.client.initiate_charge(
            reference=str(uuid4()),
            amount=500.60,
            currency=Currency.NGN,
    ...
    async def test_client_can_get_charge(self):
        response = await self.client.get_charge(
            reference="3016305c-fb05-4ef3-ad3a-96bb4c80eb7f"
        )
        self.assertTrue(codes.is_success(response.status_code))
        self.assertTrue(response.status)

    async def test_client_can_resolve_bank_account(self):
        response = await self.client.resolve_bank_account(
            bank_code="033", account_number="0000000000"
        )
        self.assertTrue(codes.is_success(response.status_code))
        self.assertTrue(response.status)

    async def test_client_can_get_balances(self):
        response = await self.client.get_balances()
        self.assertTrue(codes.is_success(response.status_code))
        self.assertEqual(response.message, "success")

    async def test_client_can_get_banks(self):
        response = await self.client.get_banks(country=Country.NIGERIA)
        self.assertTrue(codes.is_success(response.status_code))
        self.assertEqual(response.message, "Successful")

    async def test_client_can_get_mmo(self):
        response = await self.client.get_mmo(country=Country.NIGERIA)
        self.assertTrue(codes.is_success(response.status_code))
        self.assertEqual(response.message, "Successful")

    async def test_client_can_payout_to_bank_account(self):
        response = await self.client.payout_to_bank_account(
            reference=str(uuid4()),
            amount=1000,
            currency=Currency.NGN,
            bank_code="033",
    ...
    async def test_client_can_payout_to_mobile_money(self):
        response = await self.client.payout_to_mobile_money(
            reference=str(uuid4()),
            amount=1000,
            currency=Currency.KES,
    ...
    async def test_client_can_bulk_payout_to_bank_account(self):
        payout_orders = [
            {
                "reference": str(uuid4()),
                "amount": 2500,
        ...
        with self.assertRaises(ClientError) as error:
            await self.client.bulk_payout_to_bank_account(
    ...
    async def test_client_can_get_payouts(self):
        with self.assertRaises(ClientError) as error:
            await self.client.get_payouts(
    ...
    async def test_client_can_get_bulk_transaction(self):
        with self.assertRaises(ClientError) as error:
            await self.client.get_bulk_transaction(
                bulk_reference="b4cb385b-6a8b-4d61-94da-e11619490195"
            )
        self.assertEqual(
            str(error.exception),
